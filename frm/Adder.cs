...
namespace Project_for_kids.frm.Letter
{
    public partial class Adder : Form
    {
        ...
        private string imagesFolderPath = @$"{GFold.GPath}resource\\arip\\img\\";
        string soundFilePath = @$"{GFold.GPath}resource\sound\";
        ...
        public Adder()
        {
            ...
            soundPlayer = new SoundPlayer(Path.Combine(soundFilePath, "adder.wav"));
            soundPlayer.Play();
        }
            ...
            if (selectedOption == question.correctAnswer)
            {
                ...
                else
                {
                   // MessageBox.Show($"Тест завершен. Правильных ответов: {counter} из {quiz.DicQuestionValue.Count}");
                    soundPlayer = new SoundPlayer(Path.Combine(soundFilePath, $"finish.wav"));
                    soundPlayer.Play();
